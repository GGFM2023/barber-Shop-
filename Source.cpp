...
std::mutex mtx;           // mutex for shared_print
...
public:
	...
	Customer(int id, SynchronisedQueue<std::string>* queue) {
		m_id = id;
		m_queue = queue;
	}
	...
	void operator()() {
		...
		//std::cout << "Customer: " << m_id << " arrived! " <<std::endl;
		if (emptyChairs == 0)
			shared_print(str + " left, since no chair available!");
			//std::cout << "Customer: " << m_id << " left, since no chair available!" <<std::endl;
...
public:
	// Constructor with id and the queue to use
	Barber(int id, SynchronisedQueue<std::string>* queue) {
		m_id = id;
		m_queue = queue;
	}
	...
	void operator()() {
		while (a != 0) {
			if (emptyChairs == nrChairs) {
				...
				//shared_print("Barber " +std::to_string(m_id).c_str()+" begins to sleep!")
				shared_print("Barber " + std::to_string(m_id) + " begins to sleep!");

				//std::cout << "Barber " << std::to_string(m_id).c_str() << " begins to sleep!" << std::endl;
				m_cond2.wait(lock2);
				shared_print("Barber " + std::to_string(m_id) + " wakes up!");
				//std::cout << "Barber " << std::to_string(m_id).c_str() << " wakes up!" << std::endl;
			}
